const style = (function () {
  let sheet = new CSSStyleSheet()
  sheet.replaceSync(/* language=CSS */ `
    :host{
      display: inline-flex;
      gap: 0 0.2rem;
      &>div{
        flex-shrink: 0;
        position: relative;
        cursor: pointer;
        width: 1.6rem;
        height: 1.6rem;
        &::before{
          content: '';
          position: absolute;
          inset: 0.2rem;
          border-radius: 50%;
        }
        &::after{
          opacity: 0;
          transition: opacity 200ms;
          content: '';
          position: absolute;
          inset: 0.2rem;
          background-color: #171c26;
          mask-position: center center;
          mask-repeat: no-repeat;
        }
        &:nth-child(1){
          &::before{
            background-color: #FF3B30;
          }
          &::after{
            background-color: #8C1A11;
            mask-size: 0.55rem 0.55rem;
            mask-image: url("data:image/svg+xml;base64,");
          }
        }
        &:nth-child(2){
          &::before{
            background-color: #FFCC00;
          }
          &::after{
            background-color: #8F591E;
            mask-size: 0.75rem 0.15rem;
            mask-image: url("data:image/svg+xml;base64,")
          }
        }
        &:nth-child(3){
          &::before{
            background-color: #34C759;
          }
          &::after{
            background-color: #285F17;
            mask-size: 0.6rem 0.6rem;
            mask-image: url("data:image/svg+xml;base64,")
          }
        }
      }
    }
    :host(:hover)>div::after{
      opacity: 1;
    }
    :host([close]),
    :host([minimize]),
    :host([maximize]){
      &>div{
        cursor: not-allowed;
        &::before{
          background-color: #F2F2F2;
        }
        &::after{
          content: none;
        }
      }
    }
    :host([close]){
      &>div:nth-child(1){
        cursor: pointer;
        &::before{
          background-color: #FF3B30;
        }
        &::after{
          content: '';
        }
      }
    }
    :host([minimize]){
      &>div:nth-child(2){
        cursor: pointer;
        &::before{
          background-color: #34C759;
        }
        &::after{
          content: '';
        }
      }
    }
    :host([maximize]){
      &>div:nth-child(2){
        cursor: pointer;
        &::before{
          background-color: #FFCC00;
        }
        &::after{
          content: '';
        }
      }
    }
  `)
  return sheet
})()

class Control extends HTMLElement {
  #ipcRenderer = window.electron.ipcRenderer
  constructor() {
    super()
  }
  connectedCallback() {
    this.render()
  }
  render() {
    let shadow = this.attachShadow({ mode: 'open' })
    shadow.innerHTML = /* language=HTML */ `
      <div id="close" part="close"></div>
      <div id="minimize" part="minimize"></div>
      <div id="maximize" part="maximize"></div>
    `
    this.shadowRoot.getElementById('close').onclick = this.closeFun
    this.shadowRoot.getElementById('minimize').onclick = this.minimizeFun
    this.shadowRoot.getElementById('maximize').onclick = this.maximizeFun
    this.shadowRoot.adoptedStyleSheets = [style]
  }
  closeFun = () => {
    if (this.hasAttribute('close') || !(this.hasAttribute('close') || this.hasAttribute('minimize') || this.hasAttribute('maximize'))) {
      this.#ipcRenderer.send('quit')
    }
  }
  minimizeFun = () => {
    if (this.hasAttribute('minimize') || !(this.hasAttribute('close') || this.hasAttribute('minimize') || this.hasAttribute('maximize'))) {
      this.#ipcRenderer.send('minimize')
    }
  }
  maximizeFun = () => {
    if (this.hasAttribute('maximize') || !(this.hasAttribute('close') || this.hasAttribute('minimize') || this.hasAttribute('maximize'))) {
      this.#ipcRenderer.send('fullScreenToggle')
    }
  }
}

if (window.electron?.ipcRenderer && !customElements.get('wc-control')) {
  customElements.define('wc-control', Control)
}
