...
class Dialog {
  ...
  show() {
    ...
    global.set(this.element, this)
    let ontoggle = ({ newState }) => {
      if (newState === 'closed' && global.has(this.element)) {
        global.delete(this.element)
        this.element.removeEventListener('close', ontoggle)
      }
    }
...
HTMLDialogElement.prototype.show = function (event) {
  switch (this.role) {
    case 'dialog': {
      new Dialog(this, { modal: false }, event).show()
      return
    }
    case 'drawer': {
      new Drawer(this, { modal: false }).show()
      return
    }
  }
  show.call(this)
...
HTMLDialogElement.prototype.showModal = function (event) {
  switch (this.role) {
    case 'dialog': {
      new Dialog(this, { modal: true }, event).show()
      return
    }
    case 'drawer': {
      new Drawer(this, { modal: true }).show()
      return
    }
  }
  showModal.call(this)
  ...
  constructor(popover, event) {
    ...
    this.#popover.ontoggle = ({ newState }) => {
      if (newState === 'closed') {
        ...
        if (global.has(this.#popover)) global.delete(this.#popover)
      }
    }
  }
  show(event) {
    this.#target.style.removeProperty('anchor-name')
    if (event) this.#target = event.currentTarget
    this.#timer && clearTimeout(this.#timer)
    this.#target.style.anchorName = this.#popover.style.positionAnchor
    this.#timer = setTimeout(() => {
      showPopover.call(this.#popover)
    }, 200)
  }
}
...
HTMLElement.prototype.showPopover = function (event) {
  switch (this.role) {
    case 'tooltip': {
      if (!global.has(this)) global.set(this, new Tooltip(this, event))
      global.get(this).show(event)
      return
    }
  }
...
function Message(options) {
  if (!options.appendTo && !contextHolder) {
  ...
  close() {
    ...
    this.#element.ontransitionend = (e) => {
      if (e.propertyName === 'opacity' && this.#element.classList.contains('fade-out')) {
        ...
        if (contextHolder && !contextHolder.childElementCount) {
