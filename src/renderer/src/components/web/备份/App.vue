...
onMounted(() => {
  menubar.value.items = list
})
// const routes = router.options.routes.map((item) => item.path)
// function disabledMenu(list, url) {
//   url = url ?? new URL('', 'file://')
//   list.forEach((item) => {
//     let fullpath = new URL(item.pagePath, url + '/')
//     if (item.children?.length) {
//       disabledMenu(item.children, fullpath)
//     } else if (!routes.includes(fullpath.pathname)) {
//       item.disabled = true
//     }
//   })
// }
...
function routeChange(event) {
  header.value = event.detail.text
  router.push(event.detail.fullpath)
}
</script>
